...
from models.misc import MLP, Permute
from models.stochastic_depth import StochasticDepth
from models.lora import MultiRankLoRA
...
class ShiftedWindowAttention(nn.Module):
    ...
    def lorify(self, qkv_ranks, qkv_alphas, proj_ranks, proj_alphas):
        self.qkv_lora = MultiRankLoRA(self.qkv.in_features, self.qkv.out_features, qkv_ranks, qkv_alphas)
        self.proj_lora = MultiRankLoRA(self.proj.in_features, self.proj.out_features, proj_ranks, proj_alphas)
...
class SwinTransformer(nn.Module):
    def __init__(
        ...
        patch_size: List[int],
        embed_dim: int,
        depths: List[int], # depths refers to the number of Swin Transformer blocks in each stage of the hierarchical architecture
        num_heads: List[int],
        window_size: List[int],
        mlp_ratio: float = 4.0,
        dropout: float = 0.0,
    ...
    def lorify(self, qkv_ranks, qkv_alphas, proj_ranks, proj_alphas):
        ...
        assert len(proj_ranks) == len(proj_alphas), "Each rank must have a corresponding alpha."

        for i, block in enumerate(self.features):
            if isinstance(block, nn.Sequential):
                for layer in block:
                    if isinstance(layer, SwinTransformerBlock):
                        layer.lorify(qkv_ranks, qkv_alphas, proj_ranks, proj_alphas)
...
if __name__ == '__main__':
    swin_s_config = {
        ...
        "num_classes": 1000,  # Adjust if doing a different task
    }
    ...
    swin_s_model = SwinTransformer(**swin_s_config)
    swin_s_model.lorify([4, 8, 16], [0.1, 0.2, 0.4], [4], [0.1])
